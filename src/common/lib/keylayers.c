...
// Keymap dimensions (from keymaps.h)
...
// Layer state tracking (private to this module)
 ...
 */
inline uint8_t keylayers_get_state_bitmap(void) {
 ...
 */
inline bool keylayers_is_active(uint8_t layer) {
...
void keylayers_init(void) {
  ...
  if (saved_hash == 0) {
    ...
    // Keymap configuration changed - reset layer state
    ...
    // Valid saved state - but validate no invalid layers are active
...
void keylayers_process_key(uint8_t code, bool make) {
  ...
  const uint8_t target_layer = GET_LAYER_TARGET(code);  // 1-4
