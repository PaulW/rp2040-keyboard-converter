 ...
 */
static log_entry_t log_queue[UART_DMA_QUEUE_SIZE]
    __attribute__((aligned(4)));            /**< Message queue (4-byte aligned for DMA) */
...
/**< DMA channel number allocated for UART TX */
static int uart_dma_chan;  // LINT:ALLOW non-volatile - write-once during init
/**< One-time init guard */
static bool uart_dma_inited = false;  // LINT:ALLOW non-volatile - write-once during init
...
static inline bool try_reserve_slot(uint8_t* out_idx) {
    for (;;) {
        ...
        if (__atomic_compare_exchange_n(&q_head, &head, next, false, __ATOMIC_RELAXED,
                                        __ATOMIC_RELAXED)) {
            *out_idx = head;
            return true;
        }
