...
The default keymap preserves the XT keyboard layout whilst adding modern functionality through the Fn modifier.
...
**Fn Modifier Key**: F9 (default, can be customised)
...
The G80-0614H was designed for British Telecom's telex terminal system. Custom key legends are specific to terminal functions, with an XT-variant layout for telex operations. Full NKRO capability with Cherry MX switches. Good condition examples are harder to find due to the specialised application.
