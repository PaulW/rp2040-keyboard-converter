...
The hardware supports N-key rollover (NKRO).
...
The hardware supports N-key rollover (NKRO). The connector's a standard 5-pin DIN with 180° pin arrangement (same as IBM Model M Enhanced), and the cable's hardwired—no removable SDL connector.
...
The 122ST13 uses Micro Switch ST (Silent Tactile) switches—rubber domes over capacitive membrane layers. The type 13 designation in Micro Switch's naming indicates the capacitive sensing variant of the ST series, as distinct from type 22 which uses contact membrane. This capacitive design supports full N-key rollover.
