...
The Micro Switch 122ST13 is a 122-key terminal keyboard using Micro Switch ST membrane switches. Layout's similar to IBM 3270 terminal keyboards, but uses a standard 180° 5-pin DIN (AT) connector instead of the 240° arrangement found on IBM terminal keyboards.

Despite being membrane-based, the hardware supports N-key rollover (NKRO), which is unusual for membrane keyboards.
...
| **Voltage**      | 5V                                    |
| **Switch Type**  | Micro Switch ST (capacitive membrane) |
...
The Micro Switch ST switches are capacitive membrane-based with rubber domes and capacitive membrane layers. The keyboard has 122 keys with 24 function keys (F1-F24) arranged in two rows above the standard function row, full numeric keypad, and various terminal control keys—same layout pattern as IBM 3270 terminal keyboards.

The hardware supports N-key rollover (NKRO), which is unusual for membrane-based keyboards. The connector's a standard 5-pin DIN with 180° pin arrangement (same as IBM Model M Enhanced), and the cable's hardwired—no removable SDL connector.
...
To customise the key layout, edit the keymap in [`keyboard.c`](../../../src/keyboards/microswitch/122st13/keyboard.c). You can remap F13-F24 to media keys, or adjust any of the function key assignments.
...
- **Host to Device**: Converter sends configuration commands during initialisation
- **Scancode Set**: Set 3
- **Key Rollover**: N-key rollover (NKRO) - hardware capability

### Initialisation Configuration

Terminal keyboards using Set 3 are **not** in make/break mode by default. During initialisation, the converter sends command **0xF8** (Set All Keys to Make/Break) to configure the keyboard to send both key press (make) and key release (break) events for all keys. This is essential for proper operation with modern USB HID.
...
4. Wait for ACK (0xFA) confirming make/break mode is configured
...
Micro Switch (a division of Honeywell) made terminal keyboards for commercial and industrial use. They competed with Cherry and IBM in the terminal keyboard market, producing keyboards for IBM 3270 and 5250 compatible systems.

The 122ST13 uses Micro Switch ST (Silent Tactile) switches—rubber domes over capacitive membrane layers. The type 13 designation in Micro Switch's naming indicates the capacitive sensing variant of the ST series, as distinct from type 22 which uses contact membrane. The capacitive design is what supports full N-key rollover, which is unusual for rubber dome keyboards.
...
| ------------------ | ------------------------ | --------------------------------------------- | --------------------------- |
| **122ST13-2**      | IBM 3179/3180 compatible | IBM 3179/3180 plug compatible, audio feedback | Catalog model               |
...
| **122ST13S-25E**   | AT/PS2 protocol          | Standard AT interface, blue plungers          | Documented example          |
| **122ST33S-46E-J** | Unknown variant          | Telex-branded, insufficient documentation     | Contact type not documented |

**Plunger Colours**: DIN-compliant switches used black plungers for Honeywell mount keycaps or blue plungers for blade mount keycaps.
...
- Membrane contacts can oxidise—clean with isopropyl alcohol
