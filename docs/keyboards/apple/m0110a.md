...
| **Voltage**      | 5V                                                                   |
| **Switch Type**  | Alps SKCC (early), Mitsumi KAM or SMK Tall Stem (later) — all linear |
...
These keyboards used mechanical switches that varied by manufacturer and production period. Units manufactured by Alps have SKCC switches, those by Mitsumi have KAM (Mechanical Type 1) switches, and SMK-manufactured units have Tall Stem upright switches. All variants use linear switches (with the exception of Caps Lock, which uses a latching variant).
