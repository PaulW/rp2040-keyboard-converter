...
The M0110A uses mechanical switches — Alps SKCC, Mitsumi KAM, or SMK depending on production variant. It has 81 keys in Apple's extended layout, adding a numeric keypad and arrow keys to the original compact M0110 design whilst keeping the integrated cable and telephone-style RJ9/RJ10 connector.

The keyboard is limited to 2-key rollover (2KRO) — only two non-modifier keys can be reliably detected simultaneously. The Apple M0110 protocol is poll-based and proprietary: the host polls the keyboard for scancodes rather than the keyboard sending them asynchronously. There are no host-to-keyboard commands for LED control, so Caps Lock state is handled entirely on the host side.

The keymap has a single layer with no programmable layer switching.
...
| **Voltage**      | 5V                                                                                   |
| **Switch Type**  | Mechanical switches — Alps SKCC, Mitsumi KAM, or SMK depending on production variant |
...
This keyboard uses custom command mode keys because both Shift keys emit the same scancode, so the firmware cannot distinguish Left Shift from Right Shift. The default Left Shift + Right Shift combination won't work, so it uses **Left Shift + Left Alt (Option)** instead. Hold these for 3 seconds to enter Command Mode.
...
These keyboards used mechanical switches — Alps SKCC, Mitsumi KAM, or SMK depending on production variant.
...
Don't use random telephone cables—verify the 4P4C pinout first, and test continuity if you're having issues. These cables can have broken wires. The M0110A needs a power cycle to initialise properly too.
