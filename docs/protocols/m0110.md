...
- Each command-response exchange is independent in timing and framing, but the keyboard retains protocol-level state across polls (for example, the Inquiry command reports key changes since the last inquiry)
...
- Pin 2: CLOCK - Serial clock line (driven by keyboard only; host must not drive CLOCK)
...
The open-drain architecture with external pull-ups provides the following signal characteristics:

- CLOCK is driven exclusively by the keyboard; pull-ups restore the line HIGH when not driven
- DATA is driven by whichever side is transmitting; the idle side releases the line
- Rise times are determined by the pull-up resistor values and line capacitance
...
- This implementation: ~5–6ms per cycle (3.20ms host→keyboard + 2.64ms keyboard response, continuous polling with no deliberate inter-poll delay)
...
**Responses:** See Model Identification Responses table above (0x03, 0x09, 0x0B, 0x11, 0x13, 0x19, 0x1B). Any other model ID byte is treated as unknown: it is logged at DEBUG level and initialisation continues — the keyboard reset and startup sequence proceed normally, so an unlisted ID on hardware is accepted and recorded rather than causing a failure.
...
**Timing Note:** The original Macintosh ROM required the full 1000ms delay (from Apple technical documentation). Modern implementations can use shorter delays, but 1000ms is used here as a conservative compatibility margin.

### Normal Operation

**The Continuous Polling Loop:**

Modern implementation (continuous polling, no deliberate inter-poll delay):

1. Host sends Inquiry (`0x10`) — takes ~3.20ms to transmit
2. Keyboard responds: `0x7B` (nothing), `0x00-0x7F` (key pressed), or `0x80-0xFF` (key released) — response takes ~2.64ms
3. Host processes the response and updates the HID report
4. Repeat immediately

Each full poll cycle takes approximately 5–6ms, set by the protocol bit timings rather than any added software delay.
...
- Automatic retry via `keyboard_interface_task()` using the model-identification retry path, up to 5 attempts at fixed 500ms intervals
...
- Delegates to event processor: handles initialisation-state transitions (UNINITIALISED / INIT_MODEL_REQUEST / INITIALISED), filters null responses (`0x7B`), schedules follow-up Inquiry commands
- Enqueues normal key-event bytes to the ring buffer (lock-free producer)
...
| ---------------------- | ------------------------------ | --------------------------------------------------------- |
| No response            | Power issue, bad connections   | Check +5V on Pin 4, GND on Pin 1, verify pull-ups         |
...
1. **[`keyboard_interface.pio`](../../src/protocols/apple-m0110/keyboard_interface.pio)** - RP2040 PIO state machine for M0110 protocol
2. **[`keyboard_interface.c`](../../src/protocols/apple-m0110/keyboard_interface.c)** - C implementation, state machine, and command handling
3. **[`keyboard_interface.h`](../../src/protocols/apple-m0110/keyboard_interface.h)** - Protocol constants and definitions
4. **[Scancode Sets](../scancodes/README.md)** - M0110 scancode translation and processor documentation
5. **[Architecture](../advanced/README.md)** - System architecture details
