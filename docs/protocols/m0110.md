...
- Deterministic timing means completely predictable behavior
...
The host sends a command to the keyboard. The transmission is initiated by the host but clocked by the keyboard. The host's data transmission is synchronized with the keyboard's clock.
...
**Side Effect:** Resets the keyboard (clears key buffer, resets modifiers, reinitializes everything)
...
- Transmits commands synchronized to keyboard CLOCK
...
**Initialization** (from [`keyboard_interface.pio`](../../src/protocols/apple-m0110/keyboard_interface.pio)):
