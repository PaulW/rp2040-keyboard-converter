...
Run the lint script before every commit: `./tools/lint.sh` must pass with zero errors and zero warnings. The script scans for blocking operations (`sleep_ms`, `busy_wait_us`), multicore API usage (`multicore_*`, `core1_*`), printf calls in interrupt context, references to docs-internal in public files, proper volatile variable usage, and correct memory barrier placement. The script provides detailed error messages showing exactly which line violates which rule, making fixes pretty simple.
