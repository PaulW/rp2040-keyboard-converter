...
I've always preferred older keyboards and mice. Modern replacements are readily available, but I find older hardware more satisfying to use — I prefer the feel, weight, and solid construction that I just don't find in modern replacements I've tried.

I've always felt that older keyboards and mice have a certain charm and quality about them. The way they feel to use—even older membrane keyboards feel much more robust and solid compared to modern equivalents I've used (I weirdly like the feel of my Alps CL18821). To me, they just feel like they were built to last.
...
When I started looking into keyboard converters, the projects I came across—TMK, QMK, and Soarer's Converter—were built for AVR microcontrollers such as the ATmega32U4. I hadn't worked with AVR boards before, and was already exploring what I could do with the RP2040, so I decided to see if I could build something for that instead. With finding an old IBM Model F Keyboard in my parents loft, I thought it would be a fun challenge to see if I could build something using an RP2040 board instead (I wanted to learn more about how the PIO state machines worked, having already played about with the Raspberry Pi and its GPIO capabilities previously).
...
Now, there has since been work going on to support platforms such as QMK on the RP2040 (Markus Fritsche, for example, has been adding support [on his QMK fork](https://github.com/marfrit/qmk_firmware/tree/pio_m4_converter/keyboards/converter/pio_ps2_converter)). This started around the same time I did this project, and it's useful to see how that has evolved over time. Credit to Markus and others working on that. I'm certainly not trying to compete with it, or re-invent the wheel—I just wanted to build something that suited my own needs and share it with others who might find it useful.
