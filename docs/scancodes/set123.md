...
All three scancode sets use `0xAA` (self-test passed) and `0xFC` (self-test failed) for keyboard initialisation. The protocol layer filters these codes during the initialisation states (UNINITIALISED, INIT_AWAIT_SELFTEST), but the scancode processor provides defense-in-depth by logging any arrival of `0xAA` or `0xFC` during normal operation without treating them as key events.

**Set 1** requires explicit filtering due to collision: `0xAA` equals the Left Shift break code (`0x2A | 0x80 = 0xAA`). Without filtering, any `0xAA` appearing post-initialisation (keyboard reset, power glitch) would trigger a spurious Shift key release. The processor explicitly checks for and logs these codes as `!INIT!` for debugging visibility. See [Set 1 Self-Test Code Collision](set1.md#self-test-code-collision) for complete details.
