...
All three scancode sets use `0xAA` (self-test passed) and `0xFC` (self-test failed) for keyboard initialization. The protocol layer filters these during normal initialization, but the scancode processor provides defense-in-depth for post-initialization scenarios (hot-plug, unstable connections).

**Set 1** requires explicit blacklisting due to collision: `0xAA` equals the Left Shift break code (`0x2A | 0x80`). Without filtering, hot-plug events would trigger spurious Shift key releases. See [Set 1 Self-Test Code Collision](set1.md#self-test-code-collision) for complete details.

**Set 2/3** use F0 prefix for breaks, so no collision occurs. The unified processor logs these codes as potential re-plug events (following TMK's implementation) but doesn't process them as key events.
