...
The unified processor's state machine needs 11 states to handle all three sets—more than any individual set, but still quite manageable.
...
The unified processor has a more complex state machine (11 states vs up to 9 for Set 2 alone), but introduces minimal runtime branching overhead beyond the configuration pointer.
...
**Minimal overhead compared to original implementations:**

- Same O(1) lookup tables
- Inline helper functions
- Static const configuration (~4 bytes stack overhead)
- No dynamic allocation
- Minimal runtime branching overhead
