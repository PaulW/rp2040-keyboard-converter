...
**Set 1** has a collision: `0xAA` equals the Left Shift break code (`0x2A | 0x80 = 0xAA`). The Set 1 processor (individual and unified) has **no** explicit check or logging for `0xAA`/`0xFC`—they are silently processed as break codes (`0xAA` → Left Shift release). Protocol-layer filtering during initialisation is therefore the sole protection. See [Set 1 Self-Test Code Collision](set1.md#self-test-code-collision) for complete details.
